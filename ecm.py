...
class ECM:
    ...
    self.longterm_model (sm.OLS object): A model for the cointegration vector (longterm dependency).
    ...
    def fit(self, X, y):
        data = X.copy()
        self.longterm_model = sm.OLS(y, sm.add_constant(data)).fit()
    ...
    def predict(self, X, X_start, y_start):
        ...
        X_L = X_start[X_start.index.isin(self.longterm_model.params.index[1:])]
        y_L = y_start

        predictions = [y_start]
        for i in range(0, len(X)):
            cointegration_vector = y_L - (self.longterm_model.params['const'] + self.longterm_model.params.iloc[1:]@X_L)
            d_X = X.loc[:, self.longterm_model.params.index[1:]].iloc[i] - X_L
            ...
            y_L = y_hat
            X_L = X.loc[:, self.longterm_model.params.index[1:]].iloc[i]
